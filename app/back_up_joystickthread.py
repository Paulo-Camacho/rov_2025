...
class JoystickThread(QThread):
    ...
    def check_joystick_input(self):
        ...
        if self.__joystick is not None and pygame.joystick.get_count() > 0:
            ...
            left_trigger = self.__joystick.get_axis(0)
            right_trigger = self.__joystick.get_axis(0)

            # Left Joystick 4/5 this achieves the desired behavior
            left_thumbstick_left_right = self.__joystick.get_axis(4)  # j6 (Axis 4) → Left Stick Left/Right
            left_thumbstick_up_down = self.__joystick.get_axis(5)     # j8 (Axis 5) → Left Stick Up/Down

            # Right Joystick
            right_thumbstick_left_right = self.__joystick.get_axis(1)  # Default (Axis 0) → Right Stick Left/Right
            right_thumbstick_up_down = self.__joystick.get_axis(0)     # j2 (Axis 1) → Right Stick Up/Down
            ...
            if abs(right_trigger) < DEADZONE_MIN:
                right_trigger = 0
            if abs(left_thumbstick_left_right) < DEADZONE_MIN:  # j6
                left_thumbstick_left_right = 0
            if abs(left_thumbstick_up_down) < DEADZONE_MIN:     # j8
                left_thumbstick_up_down = 0
            if abs(right_thumbstick_up_down) < DEADZONE_MIN:    # j2
                right_thumbstick_up_down = 0

            axis_info = {
            "tLeft_LeftRight": left_thumbstick_left_right,  # j6 (Axis 4)
            "tLeft_UpDown": left_thumbstick_up_down,        # j8 (Axis 5)
            "tRight_LeftRight": right_thumbstick_left_right,  # Default (Axis 0)
            "tRight_UpDown": right_thumbstick_up_down        # j2 (Axis 1)
            } 
